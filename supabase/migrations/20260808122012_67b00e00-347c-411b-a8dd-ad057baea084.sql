...
ALTER TABLE public.profiles
  ADD COLUMN IF NOT EXISTS daily_word_count integer NOT NULL DEFAULT 10,
  ADD COLUMN IF NOT EXISTS vocab_last_generated date,
  ADD COLUMN IF NOT EXISTS vocab_last_test_date date,
  ADD COLUMN IF NOT EXISTS vocab_setup_done boolean NOT NULL DEFAULT false,
...
CREATE POLICY "No client read of api keys"
  ON public.gemini_keys FOR SELECT TO authenticated, anon USING (false);
CREATE POLICY "No client update of api keys"
  ON public.gemini_keys FOR UPDATE TO authenticated, anon USING (false) WITH CHECK (false);
CREATE POLICY "No client delete of api keys"
  ON public.gemini_keys FOR DELETE TO authenticated, anon USING (false);
CREATE POLICY "No client insert of api keys"
  ON public.gemini_keys FOR INSERT TO anon, authenticated WITH CHECK (false);
