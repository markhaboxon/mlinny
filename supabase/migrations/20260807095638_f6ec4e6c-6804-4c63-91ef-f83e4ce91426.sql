...
DROP POLICY IF EXISTS "teacher manages own groups" ON public.groups;
CREATE POLICY "teacher manages own groups" ON public.groups FOR ALL TO authenticated
  USING (auth.uid() = teacher_id) WITH CHECK (auth.uid() = teacher_id);
DROP POLICY IF EXISTS "member reads own group" ON public.groups;
...
DROP POLICY IF EXISTS "leave or remove member" ON public.group_members;
...
DROP POLICY IF EXISTS "student reads assignments" ON public.assignments;
...
DROP POLICY IF EXISTS "student completes own" ON public.assignment_completions;
CREATE POLICY "student completes own" ON public.assignment_completions FOR INSERT TO authenticated
  WITH CHECK (student_id = auth.uid());
DROP POLICY IF EXISTS "read completions" ON public.assignment_completions;
...
DROP POLICY IF EXISTS "undo own completion" ON public.assignment_completions;
...
DROP POLICY IF EXISTS "teacher manages materials" ON public.teacher_materials;
CREATE POLICY "teacher manages materials" ON public.teacher_materials FOR ALL TO authenticated
  USING (auth.uid() = teacher_id) WITH CHECK (auth.uid() = teacher_id);
DROP POLICY IF EXISTS "student reads group materials" ON public.teacher_materials;
...
DROP POLICY IF EXISTS "teacher manages curriculum" ON public.curriculum_entries;
CREATE POLICY "teacher manages curriculum" ON public.curriculum_entries FOR ALL TO authenticated
  USING (auth.uid() = teacher_id) WITH CHECK (auth.uid() = teacher_id AND public.is_group_teacher(auth.uid(), group_id));
DROP POLICY IF EXISTS "student reads curriculum" ON public.curriculum_entries;
...
GRANT EXECUTE ON FUNCTION public.teacher_group_students(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.teacher_group_summary(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.teacher_group_top_mistakes(uuid, int) TO authenticated;
GRANT EXECUTE ON FUNCTION public.teacher_group_activity(uuid, int) TO authenticated;
GRANT EXECUTE ON FUNCTION public.teacher_student_activity(uuid, int) TO authenticated;
GRANT EXECUTE ON FUNCTION public.teacher_student_mistakes(uuid, int) TO authenticated;
GRANT EXECUTE ON FUNCTION public.teacher_groups_overview() TO authenticated;
GRANT EXECUTE ON FUNCTION public.teacher_weekly_report(uuid) TO authenticated;